...
namespace Insight.Database.Schema
{
	...
	internal class SqlParser
	{
		...
		static SqlParser()
		{
			...
			parsers.Add(new SqlParser(SchemaObjectType.Permission, String.Format(CultureInfo.InvariantCulture, @"GRANT\s+(?<permission>(\s*,?\s*(\w+))+)\s+ON\s+(?<name>{0})\s+TO\s+(?<grantee>{0})", SqlNameExpression), "$1 ON $2 TO $3"));
